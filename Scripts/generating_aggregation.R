...
#' @author Rafael Lopes, IFT - UNESP/ISGlobal, now at Yale University
...
generating_aggregation<-function(climate_file, # Any Climate object, 'rasterBrick' objects.
  ...
  if(is.null(year)){
    cat("No year specified\n")
  }
  
  # Parameter Checks
  if(missing(climate_file) | missing(adm_limits)){
    cat("Missing Climate files, and/or Administrative level to crop!")
  }
  ...
  adm_limits_list_year<-
    foreach::foreach (i=1:layers, 
      ...
      # 
      adm_limits <- cbind(adm_limits,
                          setNames(exact_extract(dmean, adm_limits, function(values, coverage_frac, weights) {
      ...
      adm_limits <- cbind(adm_limits,
                          setNames(exact_extract(dmean, adm_limits, function(values, coverage_frac, weights) {
                            weighted.stdev(values, coverage_frac * weights)
                          },
                          weights = area(dmean),
                          stack_apply = TRUE),
                          indexes))
